...
const debug = require('debug')('fpm-plugin-mongo');
...
module.exports = {
  bind: (fpm) => {
    ...
    fpm.registerAction('INIT', () => {
      ...
      debug('The runtime finally config: %O', config);
      if(!!config.autoconnect){
        const connstr = config.connstr || `mongodb://${ config.auth.user }:${ config.auth.password }@${ config.host }:${ config.port }`
        MongoClient.connect(connstr, {
          ...
          .then( client => {
            ...
            debug('The connection %O', Vars.client.s) 
            debug('The default is db %O', Vars.db? Vars.db.s.databaseName: 'nop') 
          })
          .catch( error => {
        ...
        debug('Warning: you did disable the mongo client connect the dbserver auto! you should run connect later!')
      }
      ...
      _.extend(obj, {
        use: args => {
          debug('[use] => Arguments: %O', args)
          Vars.db = Vars.client.db(args.dbname);
          return 1;
        },
        connect: async args => {
          debug('[connect] => Arguments: %O', args)
          const { connstr = config.connstr || 'mongodb://admin:admin@localhost/testDB', db = config.default || 'testDB' } = args;
          try {
